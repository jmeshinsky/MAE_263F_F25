"""
Code By Beom Jun Kim and John Meshinsky 
...
def create_bowtie_lattice(L_cell, N_per_edge, nx_cells, ny_cells, waist_ratio=0.75):
    """
    Creates a bow-tie (hourglass) lattice like Fig. (c).
    Each cell has four corner nodes + two middle waist nodes (top/bottom).

    Parameters
    ----------
    L_cell : float
        Base cell width/height (square cell assumption)
    N_per_edge : int
        Reference number of nodes per edge (controls segment density)
    nx_cells, ny_cells : int
        Number of cells in x and y directions
    waist_ratio : float
        Indentation depth as fraction of cell height (0~0.5 recommended)

    Returns
    -------
    nodes : (N, 2) ndarray
        All node (x, y) coordinates
    beam_list : list[list[int]]
        List of beams (each a list of node indices along the beam)
    boundary_info : dict
        Contains 'bottom_nodes' and 'top_nodes'
    """

    cell_w = L_cell
    cell_h = L_cell
    vertical_spacing = cell_h * 0.25  # 셀 간 세로 간격 (셀 높이의 0.5배)
    waist_delta = waist_ratio * cell_h
    ...
    def get_or_create_node(pos):
        """Return index of existing node close to pos, or create new."""
        for existing_pos, idx in node_map.items():
            if np.linalg.norm(np.array(pos) - np.array(existing_pos)) < tol:
                return idx
        idx = len(all_nodes)
        all_nodes.append(np.array(pos, dtype=float))
        node_map[tuple(pos)] = idx
        return idx

    def create_beam(start, end, N, exclude_positions=None):
        """Create one beam from start to end with N nodes.
        ...
        exclude_positions : list of array-like, optional
            Positions to exclude (will create new nodes with small offset instead)
        """
        beam_nodes = []
        start = np.array(start, dtype=float)
        end = np.array(end, dtype=float)
        exclude_positions = exclude_positions or []
        exclude_positions = [np.array(pos, dtype=float) for pos in exclude_positions]

        for i in range(N):
            t = i / (N - 1)
            p = start + t * (end - start)

            # Check if this position is too close to any excluded position
            skip = False
            for excl_pos in exclude_positions:
                if np.linalg.norm(p - excl_pos) < tol:
                    skip = True
                    break

            if skip:
                # Create a new node with small x-offset to avoid the excluded position
                p_offset = p.copy()
                p_offset[0] += cell_w * 0.0001  # Small x-offset
                idx = get_or_create_node(p_offset)
            else:
                idx = get_or_create_node(p)
            beam_nodes.append(idx)
        return beam_nodes

    def create_beam_auto(start, end, exclude_positions=None):
        """Create beam with N_per_edge nodes (uses outer scope N_per_edge parameter).
        ...
        start, end : array-like
            Start and end positions
        exclude_positions : list of array-like, optional
            Positions to exclude from the beam
        """
        start = np.array(start, dtype=float)
        end = np.array(end, dtype=float)
        N = N_per_edge  # Use N_per_edge from outer scope
        return create_beam(start, end, N, exclude_positions=exclude_positions)

    # Keep track of waist nodes for vertical connections between rows
    top_mid_idx_grid = [[None for _ in range(nx_cells)] for _ in range(ny_cells)]
    bottom_mid_idx_grid = [[None for _ in range(nx_cells)] for _ in range(ny_cells)]

    # 2) Internal bow-tie beams within each cell (horizontal connectivity)
    for j in range(ny_cells):
        for i in range(nx_cells):
            # 각 셀의 y 위치 계산 (셀 간 간격 고려)
            y_cell_bottom = j * (cell_h + vertical_spacing)  # 셀의 아래쪽 bow 위치
            y_cell_top = y_cell_bottom + cell_h              # 셀의 위쪽 bow 위치

            # Corners of this cell
            BL = np.array([i * cell_w,         y_cell_bottom        ], dtype=float)
            BR = np.array([(i + 1) * cell_w,   y_cell_bottom        ], dtype=float)
            TL = np.array([i * cell_w,        y_cell_top   ], dtype=float)
            TR = np.array([(i + 1) * cell_w,  y_cell_top   ], dtype=float)

            # Middle waist nodes
            cx = 0.5 * (BL[0] + BR[0])
            top_mid_pos = np.array([cx, y_cell_top - waist_delta], dtype=float)
            bottom_mid_pos = np.array([cx, y_cell_bottom + waist_delta], dtype=float)

            top_mid_idx = get_or_create_node(top_mid_pos)
            bottom_mid_idx = get_or_create_node(bottom_mid_pos)

            top_mid_idx_grid[j][i] = top_mid_idx
            bottom_mid_idx_grid[j][i] = bottom_mid_idx

            # Top bow edge: TL -> top_mid -> TR
            beam_tl = create_beam_auto(TL, top_mid_pos)
            beam_list.append(beam_tl)
            beam_tr = create_beam_auto(top_mid_pos, TR)
            beam_list.append(beam_tr)

            # Bottom bow edge: BL -> bottom_mid -> BR
            beam_bl = create_beam_auto(BL, bottom_mid_pos)
            beam_list.append(beam_bl)
            beam_br = create_beam_auto(bottom_mid_pos, BR)
            beam_list.append(beam_br)

            # Left vertical edge: TL -> BL
            beam_left = create_beam_auto(TL, BL)
            beam_list.append(beam_left)

            # Right vertical edge: TR -> BR
            beam_right = create_beam_auto(TR, BR)
            beam_list.append(beam_right)

    # 3) Vertical connections between rows: connect bottom_mid(j) to top_mid(j-1)
    # This creates vertical beams connecting cells in adjacent rows
    for j in range(1, ny_cells):
        for i in range(nx_cells):
            # Connect j-th row's bottom_mid to (j-1)-th row's top_mid
            bottom_mid_pos = all_nodes[bottom_mid_idx_grid[j][i]]
            top_mid_prev_pos = all_nodes[top_mid_idx_grid[j - 1][i]]

            # Create vertical beam directly between these two waist nodes
            beam_vertical = create_beam_auto(bottom_mid_pos, top_mid_prev_pos)
            beam_list.append(beam_vertical)

    nodes = np.vstack(all_nodes)

    # 4) Identify top/bottom boundary nodes
    y_coords = nodes[:, 1]
    min_y = np.min(y_coords)
    max_y = np.max(y_coords)
    height_range = max_y - min_y

    tol_rel = height_range * 0.001
    tol_boundary = max(tol_rel, L_cell * 1e-6)

    bottom_nodes = [i for i in range(len(nodes)) if abs(nodes[i, 1] - min_y) < tol_boundary]
    top_nodes = [i for i in range(len(nodes)) if abs(nodes[i, 1] - max_y) < tol_boundary]

    # Fallback if tolerance too strict
    if len(bottom_nodes) == 0:
        tol_fb = height_range * 0.01
        bottom_nodes = [i for i in range(len(nodes)) if abs(nodes[i, 1] - min_y) < tol_fb]
    if len(top_nodes) == 0:
        tol_fb = height_range * 0.01
        top_nodes = [i for i in range(len(nodes)) if abs(nodes[i, 1] - max_y) < tol_fb]
...
def gradEb(xkm1, ykm1, xk, yk, xkp1, ykp1, curvature0, l_k, EI):
    """
    Returns the derivative of bending energy with strain-stiffening.

    Parameters:
    -----------
    xkm1, ykm1, xk, yk, xkp1, ykp1 : float
        Node coordinates
    curvature0 : float
        Reference curvature
    l_k : float
        Voronoi length
    EI : float
        Bending stiffness (constant, no strain-stiffening)
    """
    ...
    # Safety check: avoid division by zero when chi is close to 0
    if abs(chi) < eps:
        return np.zeros(6)
    ...
    dF = gradKappa * EI_eff * dkappa / l_k
    return dF

def hessEb(xkm1, ykm1, xk, yk, xkp1, ykp1, curvature0, l_k, EI):
    """
    Returns the Hessian of bending energy with strain-stiffening.
    ...
    dkappa = kappa1 - kappaBar
    dJ = 1.0 / l_k * EI_eff * np.outer(gradKappa, gradKappa)
    dJ += 1.0 / l_k * dkappa * EI_eff * DDkappa1
    return dJ

def gradEs(xk, yk, xkp1, ykp1, l_k, EA):
    """
    Calculate the gradient of the stretching energy with strain-stiffening.

    Parameters:
    -----------
    xk, yk, xkp1, ykp1 : float
        Node coordinates
    l_k : float
        Reference length
    EA : float
        Axial stiffness (constant, no strain-stiffening)
    """
    # Calculate current length
    L_current = np.sqrt((xkp1 - xk)**2.0 + (ykp1 - yk)**2.0)

    # Safety check: avoid division by zero
    eps = 1e-10
    if L_current < eps or l_k < eps:
        return np.zeros(4)
    ...
    F = np.zeros(4)
    F[0] = -(1.0 - L_current / l_k) * \
           (L_current)**(-1.0) / l_k * (-2.0 * xkp1 + 2.0 * xk)
    F[1] = -(1.0 - L_current / l_k) * \
           (L_current)**(-1.0) / l_k * (-2.0 * ykp1 + 2.0 * yk)
    F[2] = -(1.0 - L_current / l_k) * \
           (L_current)**(-1.0) / l_k * (2.0 * xkp1 - 2.0 * xk)
    F[3] = -(1.0 - L_current / l_k) * \
           (L_current)**(-1.0) / l_k * (2.0 * ykp1 - 2.0 * yk)
    F = 0.5 * EA_eff * l_k * F
    return F
...
def hessEs(xk, yk, xkp1, ykp1, l_k, EA):
    """
    Returns the 4x4 Hessian of the stretching energy with strain-stiffening.
    ...
    # Simplified expressions using L_current
    J11 = (1 / L_sq / l_k**2 * (-2 * dx)**2) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * ((-2 * dx)**2) / 2 - \
          2 * (1 - L / l_k) * (L_sq)**(-0.5) / l_k
    J12 = (1 / L_sq / l_k**2 * (-2 * dy) * (-2 * dx)) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * (-2 * dx) * (-2 * dy) / 2
    J13 = (1 / L_sq / l_k**2 * (2 * dx) * (-2 * dx)) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * (-2 * dx) * (2 * dx) / 2 + \
          2 * (1 - L / l_k) * (L_sq)**(-0.5) / l_k
    J14 = (1 / L_sq / l_k**2 * (2 * dy) * (-2 * dx)) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * (-2 * dx) * (2 * dy) / 2
    J22 = (1 / L_sq / l_k**2 * (-2 * dy)**2) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * ((-2 * dy)**2) / 2 - \
          2 * (1 - L / l_k) * (L_sq)**(-0.5) / l_k
    J23 = (1 / L_sq / l_k**2 * (2 * dx) * (-2 * dy)) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * (-2 * dy) * (2 * dx) / 2
    J24 = (1 / L_sq / l_k**2 * (2 * dy) * (-2 * dy)) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * (-2 * dy) * (2 * dy) / 2 + \
          2 * (1 - L / l_k) * (L_sq)**(-0.5) / l_k
    J33 = (1 / L_sq / l_k**2 * (2 * dx)**2) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * ((2 * dx)**2) / 2 - \
          2 * (1 - L / l_k) * (L_sq)**(-0.5) / l_k
    J34 = (1 / L_sq / l_k**2 * (2 * dy) * (2 * dx)) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * (2 * dx) * (2 * dy) / 2
    J44 = (1 / L_sq / l_k**2 * (2 * dy)**2) / 2 + \
          (1 - L / l_k) * (L_sq)**(-1.5) / l_k * ((2 * dy)**2) / 2 - \
          2 * (1 - L / l_k) * (L_sq)**(-0.5) / l_k
...
# Use the same cross-section and moment of inertia definition as in this_one_works.py
L = 0.3  # Cell size (same as this_one_works.py)
R = 0.015  # Outer radius (same as this_one_works.py)
...
print("=== MATERIAL PROPERTIES (TPU, circular cross-section) ===")
...
# Create lattice (bow-tie pattern)
# Use same grid as this_one_works.py
N_per_edge = 6  # Same as this_one_works.py
nx_tri = 5
ny_tri = 3
nodes_init, beam_list, boundary_info = create_bowtie_lattice(
    L, N_per_edge, nx_tri, ny_tri, waist_ratio=0.375
)
...
plt.tight_layout()
plt.savefig('bow_lattice_initial_structure.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('bow_lattice_deformation_snapshots.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('bow_lattice_initial_vs_final.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('bow_poisson_ratio_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('bow_shock_absorption_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
