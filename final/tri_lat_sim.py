...
def create_triangle_lattice(L_edge, N_per_edge, nx_triangles, ny_triangles, apex_height_ratio=0.5):
    """
    ...
    # Triangle dimensions
    triangle_height = L_edge
    triangle_width = L_edge
    apex_height = L_edge * apex_height_ratio
    ...
    # Calculate target spacing based on the internal beams (apex to vertices)
    dist_apex_to_vertex = np.linalg.norm(np.array([0, apex_height]) - np.array([L_edge/2, triangle_height]))
    target_spacing = dist_apex_to_vertex / (N_per_edge - 1)

    # Create lattice of four-edged triangles
    for row in range(ny_triangles):
        for col in range(nx_triangles):
            # Base position for this triangle
            base_x = col * triangle_width
            base_y = row * triangle_height * 0.5  # 0.5 multiplier makes triangles overlap vertically

            # Define the 4 key vertices of this four-edged triangle
            base_left = np.array([base_x, base_y])  # Bottom left
            base_right = np.array([base_x + triangle_width, base_y])  # Bottom right
            base_top = np.array([base_x + triangle_width/2, base_y + triangle_height])  # Top vertex
            apex = np.array([base_x + triangle_width/2, base_y + apex_height])  # Internal apex

            # Calculate number of nodes for each beam
            dist_right_edge = np.linalg.norm(base_top - base_right)
            dist_left_edge = np.linalg.norm(base_top - base_left)
            dist_apex_to_left = np.linalg.norm(base_left - apex)
            dist_apex_to_right = np.linalg.norm(base_right - apex)

            N_right = max(2, int(np.round(dist_right_edge / target_spacing)) + 1)
            N_left = max(2, int(np.round(dist_left_edge / target_spacing)) + 1)
            N_apex_left = max(2, int(np.round(dist_apex_to_left / target_spacing)) + 1)
            N_apex_right = max(2, int(np.round(dist_apex_to_right / target_spacing)) + 1)

            # Create the 4 beams for this triangle
            # Beam 1: base_right to base_top (right outer edge)
            beam1 = create_beam(base_right, base_top, N_right)
            beam_list.append(beam1)

            # Beam 2: base_top to base_left (left outer edge)
            beam2 = create_beam(base_top, base_left, N_left)
            beam_list.append(beam2)

            # Beam 3: apex to base_left (left internal edge)
            beam3 = create_beam(apex, base_left, N_apex_left)
            beam_list.append(beam3)

            # Beam 4: apex to base_right (right internal edge)
            beam4 = create_beam(apex, base_right, N_apex_right)
            beam_list.append(beam4)
...
nodes_init, beam_list, boundary_info = create_triangle_lattice(
    L, N_per_edge, nx_tri, ny_tri, apex_height_ratio=0.5
)
...
plt.tight_layout()
plt.savefig('tri_lattice_initial_structure.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('tri_lattice_deformation_snapshots.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('tri_lattice_initial_vs_final.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('tri_poisson_ratio_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('tri_shock_absorption_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
